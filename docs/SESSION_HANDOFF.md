...
| `sql/add_create_roster_member_rpc.sql` | `public.create_roster_member()` SECURITY DEFINER RPC powers the "+ Add Member" button on the Personnel → Roster tab. Owner/admin/manager-gated. Atomically inserts an unlinked `public.users` row (or reuses one matching the email case-insensitively, for the cross-company case) plus a `company_memberships` row. Returns `{user_id, membership_id, existing_user}`. The roster-invite Edge Function then dispatches the invitation email to the returned membership_id. | Single-member add UI in the Roster tab |
