/**
 * Intel — Country risk + major exchanges (open/closed).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        for (const f of data.features ?? []) {
          ...
          for (const code of Object.keys(RISK_FACTORS)) {
            if (place.includes(code.toLowerCase())) {
      ...
      console.warn("[intel-country-risk] USGS error:", e);
    }

    const deriveLevel = (base: number): "CRITICAL" | "HIGH" | "ELEVATED" | "LOW" => {
    ...
    const countries = Object.entries(RISK_FACTORS)
      .map(([code, data]) => ({
        ...
        risk_level: deriveLevel(data.base),
        tags: data.tags,
      }))
    ...
    console.error("[intel-country-risk]", err);
    return new Response(
      JSON.stringify({
        countries: [],
        exchanges: [],
