...
-- Helper function for checking admin role (owner/admin/manager)
...
CREATE POLICY "shifts_delete" ON shifts FOR DELETE TO authenticated
  USING (event_id IN (SELECT id FROM events WHERE is_company_admin(company_id)));
...
CREATE POLICY "chat_members_delete" ON chat_members FOR DELETE TO authenticated
  USING (user_id IN (SELECT id FROM users WHERE supabase_id = auth.uid()::text)
    OR channel_id IN (SELECT id FROM chat_channels WHERE is_company_admin(company_id)));
...
-- Memberships: admins can remove members
...
-- Timesheets: users can delete their own
CREATE POLICY "timesheets_delete" ON timesheets FOR DELETE TO authenticated
  USING (user_id IN (SELECT id FROM users WHERE supabase_id = auth.uid()::text));

-- ============================================================
-- ✅ Setup complete! All 28 tables + indexes + RLS policies (incl. DELETE) created.
-- ============================================================
