...
export function acousticRematch(
  ...
  // Reassign if a different speaker is meaningfully closer (25% margin
  // avoids flapping on borderline cases).
  //
  // Note: previously experimented with a looser margin for boundary
  // words (0% — any closer profile wins). That over-corrected when
  // tested against pitch-modulated single-speaker recordings, which
  // aren't representative of real two-speaker audio. Uniform conservative
  // margin is the safer default until validated against real data.
  const MARGIN = 0.75;
