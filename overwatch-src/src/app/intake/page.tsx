...
function IntakeFormInner() {
  ...
  const [error, setError] = useState<string | null>(() => token ? null : "No intake token provided.");
  ...
  const renderChip = (field: "venueType" | "threatTypes" | "successCriteria", options: string[]) => (
  ...
  const renderPill = (field: keyof ClientIntakeData, options: string[]) => (
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        <section className="space-y-3"><h2 className="text-sm font-semibold flex items-center gap-2"><Shield className="h-4 w-4" style={{ color: bc }} /> Your Information</h2>
          <div className="grid grid-cols-2 gap-3"><div><label htmlFor="intake-client-name" className="sr-only">Your full name</label><Input id="intake-client-name" placeholder="Your full name *" value={data.clientName} onChange={(e) => upd("clientName", e.target.value)} /></div><div><label htmlFor="intake-client-email" className="sr-only">Email address</label><Input id="intake-client-email" placeholder="Email address *" type="email" value={data.clientEmail} onChange={(e) => upd("clientEmail", e.target.value)} /></div></div></section>
        <section className="space-y-3"><h2 className="text-sm font-semibold flex items-center gap-2"><Target className="h-4 w-4" style={{ color: bc }} /> What do you need?</h2>
          ...
          <div><label className="text-xs text-muted-foreground mb-1 block">Time Sensitivity</label>{renderPill("timeSensitivity", ["Low", "Medium", "High", "Immediate"])}</div></section>
        <section className="space-y-3"><h2 className="text-sm font-semibold flex items-center gap-2"><MapPin className="h-4 w-4" style={{ color: bc }} /> Location & Environment</h2>
          <div className="grid grid-cols-2 gap-3"><div><label htmlFor="intake-attendance" className="text-xs text-muted-foreground mb-1 block">Attendance</label><Input id="intake-attendance" placeholder="e.g., 500" value={data.estimatedAttendance} onChange={(e) => upd("estimatedAttendance", e.target.value)} /></div><div><label className="text-xs text-muted-foreground mb-1 block">Environment</label>{renderPill("environment", ["Indoor", "Outdoor", "Hybrid"])}</div></div>
          <div><label className="text-xs text-muted-foreground mb-1 block">Venue Type</label>{renderChip("venueType", VENUE_TYPES)}</div>
        ...
        <section className="space-y-3"><h2 className="text-sm font-semibold flex items-center gap-2"><AlertTriangle className="h-4 w-4" style={{ color: bc }} /> Risk & Concerns</h2>
          ...
          <div><label className="text-xs text-muted-foreground mb-1 block">Risk Level</label>{renderPill("riskLevel", ["Low", "Moderate", "High", "Critical"])}</div>
          <div><label className="text-xs text-muted-foreground mb-1 block">Threat Types</label>{renderChip("threatTypes", THREAT_TYPES)}</div></section>
        <section className="space-y-3"><h2 className="text-sm font-semibold flex items-center gap-2"><CheckCircle2 className="h-4 w-4" style={{ color: bc }} /> Success Criteria</h2>
          {renderChip("successCriteria", SUCCESS_CRITERIA)}
